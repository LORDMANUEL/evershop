...
- El repositorio mantiene la base estándar de EverShop sin personalizaciones específicas para autopartes.
- La infraestructura prevista se apoya en `docker-compose.yml`, preparada para levantar EverShop y Postgres.
- No se han creado extensiones nuevas dentro de `extensions/` ni se han modificado los temas o el frontend.
...
2. **Organizar extensiones**: Crear las carpetas `extensions/Auto_Fitment`, `Auto_VIN`, `Auto_Search`, `Payments_Local` y `Shipping_Rules`, registrarlas como workspaces y habilitarlas en los archivos de configuración.
...
5. **Construir selector de vehículo**: Implementar un widget persistente para Año/Marca/Modelo/VIN y consumir la API vPIC en una extensión dedicada.
6. **Importadores masivos**: Desarrollar herramientas en el admin para cargar CSV/Excel de catálogo, compatibilidad y cross-reference con validación.
...
8. **Pagos y envíos locales**: Adaptar métodos de pago según la región objetivo y definir reglas de envío por zona/peso.
